        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            @foreach(['income', 'expense', 'transfer'] as $t)
                <x-panel wire:key="panel-{{ $t }}" :title="ucfirst($t) . ' Categories'" :color="$t === 'income' ? 'green' : ($t === 'expense' ? 'red' : 'blue')">
                    <x-slot name="action">
                        <button type="button" wire:click="openModal(null, '{{ $t }}')" class="text-[9px] font-mono font-bold text-ink-3 hover:text-ink uppercase tracking-wider transition-colors">
                            + Add
                        </button>
                    </x-slot>
                    <div class="divide-y divide-edge">
                        ...
                        @forelse($typeCategories as $category)
                            <div wire:key="cat-row-{{ $category->id }}" class="group relative flex items-center justify-between p-2.5 hover:bg-surface-2 transition-colors">
                                <div class="flex items-center gap-2">
                                    <div class="w-2 h-2 rounded-full shrink-0" style="background-color: {{ $category->color }}"></div>
                                    <div class="min-w-0">
                                        <p class="text-xs font-medium text-ink truncate">{{ $category->name }}</p>
                                        @if($category->parent_id)
                                            <p class="text-[9px] text-ink-3 uppercase tracking-tighter">Sub of {{ $category->parent->name }}</p>
                                        @endif
                                    </div>
                                </div>

                                <div class="flex items-center gap-2 shrink-0">
                                    <span class="text-[9px] font-mono text-ink-3">{{ $category->transactions_count }} txs</span>

                                    @if($category->user_id)
                                        <div class="flex items-center gap-1 opacity-40 group-hover:opacity-100 transition-opacity">
                                            <button type="button" wire:click="openModal({{ $category->id }})" class="text-ink-3 hover:text-ink transition-colors p-0.5" title="Edit">
                                                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
                                            </button>
                                            <button type="button" wire:click="delete({{ $category->id }})" wire:confirm="Are you sure you want to delete this category?" class="text-ink-3 hover:text-finance-red transition-colors p-0.5" title="Delete">
                                                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                                            </button>
                                        </div>
                                    @else
                                        <span class="text-[9px] bg-surface-3 px-1 py-0.5 rounded text-ink-3 uppercase tracking-widest font-mono">System</span>
                                    @endif
                                </div>
                            </div>
                        @empty
                            <div wire:key="cat-none-{{ $t }}" class="p-3 text-center text-[9px] text-ink-3 italic uppercase tracking-widest">No categories</div>
                        @endforelse
                    </div>
                </x-panel>
            @endforeach
