<div>
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6 pt-6">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-xl font-medium text-ink">Manage Accounts</h1>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            @foreach($accounts as $account)
                <div wire:key="acc-{{ $account->id }}" class="bg-surface border border-edge rounded overflow-hidden shadow-sm hover:shadow-md transition-shadow">
                    ...
                    <div class="p-5">
                        <h3 class="text-lg font-medium mb-1 truncate">{{ $account->name }}</h3>
                    ...
                    <div class="px-4 py-2 bg-surface-3 border-t border-edge flex justify-between">
                         <button type="button" wire:click="openModal({{ $account->id }})" class="text-[10px] text-ink-2 hover:text-ink font-medium uppercase tracking-widest">Edit</button>
                         <button type="button" wire:click="delete({{ $account->id }})" wire:confirm="Are you sure you want to delete this account?" class="text-[10px] text-finance-red hover:text-red-700 font-medium uppercase tracking-widest">Delete</button>
                    </div>
                </div>
            @endforeach
        </div>
