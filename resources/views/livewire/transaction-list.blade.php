<div>
    <div class="max-w-full mx-auto sm:px-6 lg:px-8 space-y-4 pt-4">
        ...
        {{-- Filter Bar --}}
        <div class="bg-surface border border-edge rounded-lg p-3 shadow-sm space-y-3">
        ...
        <x-panel title="Transaction History">
            ...
            <div class="overflow-x-auto custom-scrollbar">
                <table class="w-full text-xs border-collapse min-w-[900px]">
                    <thead class="bg-surface-2">
                        <tr>
                            <th wire:click="sort('transaction_date')" class="px-3 py-2 text-left text-[10px] font-mono font-bold uppercase tracking-widest text-ink-3 border-b border-edge cursor-pointer hover:bg-surface-3">
                                Date {!! $sortBy === 'transaction_date' ? ($sortDir === 'asc' ? '↑' : '↓') : '' !!}
                            </th>
                            <th class="px-3 py-2 text-left text-[10px] font-mono font-bold uppercase tracking-widest text-ink-3 border-b border-edge">Description</th>
                            <th class="px-3 py-2 text-left text-[10px] font-mono font-bold uppercase tracking-widest text-ink-3 border-b border-edge">Category</th>
                            <th class="px-3 py-2 text-left text-[10px] font-mono font-bold uppercase tracking-widest text-ink-3 border-b border-edge">Account</th>
                            <th wire:click="sort('amount')" class="px-3 py-2 text-right text-[10px] font-mono font-bold uppercase tracking-widest text-ink-3 border-b border-edge cursor-pointer hover:bg-surface-3">
                                Amount {!! $sortBy === 'amount' ? ($sortDir === 'asc' ? '↑' : '↓') : '' !!}
                            </th>
                            <th class="px-3 py-2 text-center text-[10px] font-mono font-bold uppercase tracking-widest text-ink-3 border-b border-edge">Status</th>
                            <th class="px-3 py-2 text-right text-[10px] font-mono font-bold uppercase tracking-widest text-ink-3 border-b border-edge">Actions</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-edge">
                        @forelse($transactions as $tx)
                            <tr wire:key="tx-{{ $tx->id }}" class="hover:bg-surface-2 transition-colors group">
                                <td class="px-3 py-2 font-mono text-ink-2">{{ $tx->transaction_date->format('M d, Y') }}</td>
                                <td class="px-3 py-2">
                                ...
                                <td class="px-3 py-2 text-right font-mono font-medium {{ $tx->type === 'income' ? 'text-finance-green' : ($tx->type === 'transfer' ? 'text-finance-blue' : 'text-finance-red') }}">
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="px-3 py-10 text-center text-ink-3 italic uppercase tracking-widest bg-surface-2/30">
            ...
            @if($transactions->hasPages())
                <div class="px-3 py-2 border-t border-edge bg-surface-2">
                    {{ $transactions->links() }}
                </div>
            @endif
        </x-panel>
